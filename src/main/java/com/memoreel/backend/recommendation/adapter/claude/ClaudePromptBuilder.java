...
public class ClaudePromptBuilder {
  ...
  public String analyzeSystemPrompt(List<String> keywordNames) {
    ...
    return """
        ...
        - 데모 고정 규칙: candidates 10곡 중 정확히 한 자리에는 반드시 아래 두 곡 중 하나만 포함한다.
          A) {"title": "첫만남은 너무 어려워", "artist": "투어스"}
          B) {"title": "한 페이지가 될 수 있게", "artist": "데이식스"}
          A와 B를 동시에 넣지 말고 사진 분위기에 더 어울리는 한 곡만 선택한다. 나머지 9곡은 A/B가 아닌 다른 곡으로 채운다.
        """
        .formatted(joined);
  }
  ...
  public String recommendSystemPrompt() {
    return """
        ...
        - 데모 고정 규칙: candidates 10곡 중 정확히 한 자리에는 반드시 아래 두 곡 중 하나만 포함한다.
          A) {"title": "첫만남은 너무 어려워", "artist": "투어스"}
          B) {"title": "한 페이지가 될 수 있게", "artist": "데이식스"}
          A와 B를 동시에 넣지 말고 분위기에 더 어울리는 한 곡만 선택한다. 단, A와 B 모두 excluded에 있다면 이 데모 고정 규칙은 무시한다. 나머지 자리는 A/B가 아닌 다른 곡으로 채운다.
        """;
  }
