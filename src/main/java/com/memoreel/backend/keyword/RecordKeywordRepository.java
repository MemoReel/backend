...
public interface RecordKeywordRepository extends JpaRepository<RecordKeyword, Long> {
    ...
    /**
     * 키워드별 본인 저장 곡 수 (distinct 곡 기준, 명세 §5-1 track_count).
     * 저장이 한 건도 없는 키워드는 결과에 포함되지 않는다(서비스에서 0으로 채움).
     */
    @Query("""
    ...
    /**
     * 특정 키워드로 분류된 본인 저장 곡들의 "가장 최근 저장 record id" 목록 (명세 §5-2).
     * distinct 곡 기준이라 곡별 최신 record(MAX id)만 골라 최근순 정렬한다.
     * 커서(record id)보다 작은 것만 가져와 커서 페이지네이션을 구현한다.
     */
    @Query("""
            SELECT MAX(r.id)
            FROM RecordKeyword rk
            ...
            """)
    List<Long> findRecentRecordIds(@Param("userId") Long userId,
