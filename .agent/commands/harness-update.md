This workflow manages and updates the harness engineering infrastructure. It is the Codex equivalent of the Claude `/harness-update` command.
...
| `CLAUDE.md` | Claude agent guide; keep synchronized when shared project guidance changes |
...
| `build.gradle` | Spotless, ArchUnit dependencies, and harness task definitions |
...
1. Analyze the user request and decide which harness components need to change.
2. Read the current state of those components before editing.
3. Check for unrelated user changes with `git status --short` and avoid reverting them.
...
4. Update related harness files such as `AGENTS.md`, `CLAUDE.md`, and `MAP.md`.
...
2. Reflect the summary in both `AGENTS.md` and `CLAUDE.md`.
...
- `AGENTS.md` <-> `CLAUDE.md` <-> `docs/specs/CONVENTIONS.md`
